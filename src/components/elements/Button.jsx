...
function Button({ label, type, className, ...props }) {
    ...
    return (
        <button {...props} onMouseDown={() => setPressed(true)} onMouseUp={() => setPressed(false)} className={`flex items-center justify-center gap-2 text-xl border px-5 py-3 rounded-lg cursor-pointer transition-all ease-out ${pressed ? "scale-105" : "scale-100"} ${customClass[type]} ${className}`}>
            {type === "search" ?
